...
function modify_font_color(target: string, hook_method = "tag") {
  for (let x of hook_method === "class"
    ? document.getElementsByClassName(target)
    : document.getElementsByTagName(target)) {
    // Hook method specified like class so, op. by class else op. by tag
...
function resources_selector() {
  ...
  for (let paragraph of para) {
    ...
    for (let i = 0; i < paragraph_children.length; i++) {
      if (
        paragraph_children[i].innerHTML === "À prévoir :" ||
        paragraph_children[i].innerHTML ===
    ...
    if (list != undefined) {
      for (let puce of list.getElementsByTagName("li")) {
        ...
        puce.addEventListener("click", function (e) {
          if (
            this.style.color != "" &&
            rgb2hex(this.style.color).toUpperCase() ===
              resources_selected_color.toLocaleUpperCase()
          ) {
...
window.addEventListener("load", (event) => {
  ...
  if (document.URL.includes("https://www.dofuspourlesnoobs.com/")) {
    //We're on DPLN
    if (theme_color != "") {
      apply_theme();
    }
